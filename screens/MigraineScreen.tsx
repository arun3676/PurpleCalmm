...
// If you use typed navigation, you can adjust this later.
// Keeping the component untyped here to avoid build errors.
...
export default function MigraineScreen() {
  ...
  const [dimLevel, setDimLevel] = useState(0.6); // 0..1 overlay, can hook to a slider later
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 48 }}>
        ...
        <View style={styles.row}>
          <Pressable
            onPress={() => setSoundName('mochi')}
            style={[styles.pill, { backgroundColor: colors.accentBg }]}
          >
        ...
        <View style={styles.rowWrap}>
          {([1, 5, 10, 15, 30, 45] as const).map((m) => (
            <Pressable
              ...
              style={[
                styles.chip,
                { backgroundColor: minutes === m ? colors.accentBg : colors.card }
              ]}
            >
              <Text style={{
                color: minutes === m ? colors.accentText : colors.text
              }}>
                {m}m
              </Text>
            </Pressable>
        ...
        <View style={styles.row}>
          <Pressable
            onPress={() => setMinutes(minutesClamp(minutes - 1))}
            style={[styles.step, { backgroundColor: colors.card }]}
          >
          ...
          <Pressable
            ...
            style={[
              styles.start,
              { backgroundColor: colors.accentBg, marginLeft: 'auto' }
            ]}
          >
      ...
      {/* Ultra-dim overlay */}
      {dimOn && (
        <Pressable onPress={() => setDimOn(false)} style={[
          StyleSheet.absoluteFillObject,
          { backgroundColor: `rgba(0,0,0,${dimLevel})` }
        ]} accessibilityLabel="Exit dim mode" />
      )}

      {/* Dim toggle fixed at bottom-right */}
      <Pressable
        onPress={() => setDimOn(!dimOn)}
        style={[
          styles.fab,
          { backgroundColor: dimOn ? colors.accentBg : colors.card, borderColor: colors.border }
        ]}
        accessibilityLabel="Toggle ultra-dim"
      >
        <Text style={{ color: dimOn ? colors.accentText : colors.text }}>
          {dimOn ? 'Dim: On' : 'Dim: Off'}
        </Text>
      </Pressable>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  chip: {
    paddingVertical: 10, paddingHorizontal: 14, borderRadius: 12, marginRight: 8, marginBottom: 8
  },
